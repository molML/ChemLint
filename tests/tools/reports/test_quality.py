...
def test_full_workflow_regression(clean_subset_dataset):
    """Test complete workflow with regression activity data."""
    result_filename = _perform_quality_report_calculations(
        input_filename=clean_subset_dataset['dataset_filename'],
        project_manifest_path=clean_subset_dataset['manifest_path'],
        smiles_col='SMILES',
        ...
        activity_type='regression',
        activity_units='nM',
        output_name='full_regression_test',
        explanation='Full regression workflow test'
    )
    ...
    required_sections = [
        'metadata', 'completeness', 'smiles_validity', 'pains', 'duplicates',
        'physicochemical_properties', 'lipinski', 'veber', 'qed', 'outliers',
        'activity', 'scaffold_diversity', 'functional_groups', 'stereochemistry',
        'charge_state', 'salts_fragments', 'special_features',
...
def test_full_workflow_classification(quality_report_dataset, session_workdir):
    """Test complete workflow with classification activity data."""
    # Load dataset and filter to rows with valid labels
    df = quality_report_dataset['df'].copy()
    # Filter out rows where label is NaN or empty
    df_filtered = df[df['label'].notna()].copy()
    ...
    # Store filtered dataset
    manifest_path = session_workdir / "manifest_full_classification.json"
    manifest_data = {"resources": []}
    with open(manifest_path, 'w') as f:
        json.dump(manifest_data, f)
    
    filtered_filename = _store_resource(
        df_filtered, str(manifest_path), "filtered_full_classification_dataset", 
        "Filtered dataset for full classification workflow", "csv"
    )
    ...
    result_filename = _perform_quality_report_calculations(
        ...
        activity_col='label',
        activity_type='classification',
        output_name='full_classification_test',
        explanation='Full classification workflow test'
    )
    
    results = _load_resource(str(manifest_path), result_filename)
    
    # Verify classification-specific fields
    assert results['activity']['type'] == 'classification'
    assert 'balance' in results['activity']
    assert 'n_positive' in results['activity']
    assert 'n_negative' in results['activity']
